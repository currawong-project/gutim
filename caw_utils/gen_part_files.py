...
def gen_sf_score( cfg ):
    ...
    def _insert_bar_column(rowL):
        ...
        for r in rowL:
            ...
            if hasattr(r,'meas') and r.meas is not None and r.meas != cur_meas:
                bar_row = {}
                for v,k in bar_row.items():
                    bar_row[k] = None
                    
                bar_row['bar'] = r.meas
                outL.append(types.SimpleNamespace(**bar_row))
                cur_meas = r.meas
    ...
    def _write_score(cfg,outL):
        ...
        titleL = [ title for title,_ in SCORE_CSV_TITLES ] + ['loc','bar','src','src_meas','index']
